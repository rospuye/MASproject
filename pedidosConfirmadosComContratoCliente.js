...
var listaNomesPedidosCliente = [];
var listaIDsOrganizadores = [];
...
function displayPedidos(){
    ...
    for (i=0; i<localStorage.length; i++){
        ...
        if (item.includes('"tipo":"pedido","estado":"confirmadoComContrato"')){
            ...
            if (itemName.includes('submissaoPedido/Cliente' + clientID)){
                ...
                listaNomesPedidosCliente.push(itemName);
                var numeroPedido = parseInt(itemName.substring(32));
                numeracaoPedidos.push(numeroPedido);
                var IDOrganizador = item.substring(item.length-8, item.length-2);
                listaIDsOrganizadores.push(IDOrganizador);
            }
        }
    }
    ...
    for (i=0; i<max; i++){
        for (j=0; j<numeracaoPedidos.length; j++){
            ...
            if ((i+1)==numeracaoPedidos[j]){
                ...
                var IDOrg = listaIDsOrganizadores[j];
                ...
                // colocar o pedido em display na página
                ...
                for (p=0; p<localStorage.length; p++){
                    ...
                    if (itemName3.includes('dataConfirmacaoSemContrato - ' + listaNomesPedidosCliente[j])){
                        ...
                        p4.id = "dataSemContrato" + displayPedidosCounter;
                ...
                for (p=0; p<localStorage.length; p++){
                    var item3 = localStorage.getItem(localStorage.key(p));
                    var itemName3 = localStorage.key(p);
                    if (itemName3.includes('dataConfirmacaoComContrato - ' + listaNomesPedidosCliente[j])){
                        var rowDiv2 = document.createElement("div");
                        rowDiv2.className = "row";
                        ...
                        var p4 = document.createElement("p");   
                        p4.id = "dataComContrato" + displayPedidosCounter;
                        p4.className = "pedidosElement";
                        p4.innerText = item3.substring(9,18) + " " + item3.substring(28,36);
                        rowDiv2.appendChild(p4);
                ...
                btn.setAttribute("id", "contactarOrganizador" + numero.toString());
                btn.setAttribute("type", "button");
                btn.setAttribute("value", "Contactar Organizador");
                btn.setAttribute("class", "btn btn-light");
                btn.setAttribute("name", IDOrg);
                newDiv.appendChild(btn);

                var currentDiv = document.getElementById("todosOsPedidos"); 
                document.body.insertBefore(newDiv, currentDiv);

                document.getElementById("contactarOrganizador" + numero.toString()).onclick = function() {
                    var flag = 0;
                    ...
                    window.location.replace("mensagensClienteFunc.html");
                };
    ...
    console.log(listaIDsOrganizadores);
    console.log(listaPedidosCliente);
