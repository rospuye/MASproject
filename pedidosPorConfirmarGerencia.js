function displayPedidos(){
    ...
    for (i=0; i<localStorage.length; i++){
        ...
        if (item.includes('"tipo":"pedido","estado":"porconfirmar"')){
            nenhumPedidoFlag = 1;
            ...
            for (j=0; j<localStorage.length; j++){
                ...
                if (item2.includes('"tipo":"conta"') || item2.includes('"tipo":"login"')){
                    ...
                    if (item2.substring(indexArray3[5]+2,indexArray4[5]-1)==clientID){
                        ...
                        p16.innerText = item.substring(indexArray[16]+2, item.length-2);
                        ...
                        btn.setAttribute("id", "confirmacaoPedido" + pedidosCounter);
                        btn.setAttribute("type", "button");
                        btn.setAttribute("value", "Aceitar Pedido");
                        btn.setAttribute("class", "btn btn-light");
                        btn.setAttribute("style", "margin-right:1%");
                        newDiv.appendChild(btn);

                        var btn2 = document.createElement("input");
                        btn2.setAttribute("id", "justificacaoRejeicao" + pedidosCounter);
                        btn2.setAttribute("type", "button");
                        btn2.setAttribute("value", "Rejeitar Pedido");
                        btn2.setAttribute("class", "btn btn-light");
                        newDiv.appendChild(btn2);

                        var rowDiv15 = document.createElement("div");
                        rowDiv15.setAttribute("id", "form" + pedidosCounter);
                        rowDiv15.setAttribute("style", "display: none");

                        var form = document.createElement("form");
                        form.setAttribute("id", "formDiv" + pedidosCounter);
                        var formGroup = document.createElement("div");
                        formGroup.className = "form-group";
                        var label = document.createElement("label");
                        label.setAttribute("for", "confirmacaoInput");
                        label.setAttribute("class", "col-form-label text-right");
                        label.innerText = "Confirmação do pedido";
                        formGroup.appendChild(label);
                        var confirmacaoDiv = document.createElement("div");
                        confirmacaoDiv.className = "col-md-8";
                        var confirmacaoInput = document.createElement("textarea");
                        confirmacaoInput.setAttribute("rows", "8");
                        confirmacaoInput.setAttribute("name", "confirmacaoInput");
                        confirmacaoInput.setAttribute("id", "confirmacaoInput" + pedidosCounter);
                        confirmacaoInput.setAttribute("class", "form-control");
                        confirmacaoDiv.appendChild(confirmacaoInput);
                        formGroup.appendChild(confirmacaoDiv);
                        form.appendChild(formGroup);
                        var btn3 = document.createElement("input");
                        btn3.setAttribute("id", "aceitarPedido" + pedidosCounter);
                        btn3.setAttribute("type", "button");
                        btn3.setAttribute("value", "Enviar");
                        btn3.setAttribute("class", "btn btn-dark");
                        btn3.setAttribute("style", "margin-right:1%");
                        newDiv.appendChild(btn3);
                        form.appendChild(btn3);
                        rowDiv15.appendChild(form);

                        newDiv.appendChild(rowDiv15);

                        var rowDiv16 = document.createElement("div");
                        rowDiv16.setAttribute("id", "2form" + pedidosCounter);
                        rowDiv16.setAttribute("style", "display: none");

                        var form2 = document.createElement("form");
                        form2.setAttribute("id", "2formDiv" + pedidosCounter);
                        var formGroup2 = document.createElement("div");
                        formGroup2.className = "form-group";
                        var label2 = document.createElement("label");
                        label2.setAttribute("for", "justificacaoInput");
                        label2.setAttribute("class", "col-form-label text-right");
                        label2.innerText = "Justificação da rejeição";
                        formGroup2.appendChild(label2);
                        var justificacaoDiv = document.createElement("div");
                        justificacaoDiv.className = "col-md-8";
                        var justificacaoInput = document.createElement("textarea");
                        justificacaoInput.setAttribute("rows", "8");
                        justificacaoInput.setAttribute("name", "justificacaoInput");
                        justificacaoInput.setAttribute("id", "justificacaoInput" + pedidosCounter);
                        justificacaoInput.setAttribute("class", "form-control");
                        justificacaoDiv.appendChild(justificacaoInput);
                        formGroup2.appendChild(justificacaoDiv);
                        form2.appendChild(formGroup2);
                        var btn4 = document.createElement("input");
                        btn4.setAttribute("id", "rejeitarPedido" + pedidosCounter);
                        btn4.setAttribute("type", "button");
                        btn4.setAttribute("value", "Enviar");
                        btn4.setAttribute("class", "btn btn-dark");
                        btn4.setAttribute("style", "margin-right:1%");
                        newDiv.appendChild(btn4);
                        form2.appendChild(btn4);
                        rowDiv16.appendChild(form2);

                        newDiv.appendChild(rowDiv16);
                        ...
                        document.getElementById("confirmacaoPedido" + pedidosCounter).onclick = function() {
                            document.getElementById("form" +  this.id.substring(17)).style.display = "block";
                            document.getElementById("2form" +  this.id.substring(17)).style.display = "none";
                        };
                        document.getElementById("justificacaoRejeicao" + pedidosCounter).onclick = function() {
                            document.getElementById("form" +  this.id.substring(20)).style.display = "none";
                            document.getElementById("2form" +  this.id.substring(20)).style.display = "block";
                        };
                        document.getElementById("aceitarPedido" + pedidosCounter).onclick = function() {
                            aceitarPedido(parseInt(this.id.substring(13)), confirmacaoInput.value.trim())
                        };
                        document.getElementById("rejeitarPedido" + pedidosCounter).onclick = function() 
                        {rejeitarPedido(parseInt(this.id.substring(14)), justificacaoInput.value.trim())
                        };
                    }
                }
            }
...
function rejeitarPedido(num, text){
    var pedidosCounter2 = 0;
    for (i=0; i<localStorage.length; i++){
        var item = localStorage.getItem(localStorage.key(i));
        var itemName = localStorage.key(i);
        if (item.includes('"tipo":"pedido","estado":"porconfirmar"')){
            pedidosCounter2++;
            console.log(pedidosCounter2, num);
            if (pedidosCounter2==num){
                ...
                var lastPart = item.substring(indexArray2[25], item.length-1);
                var newItem = firstPart + '"rejeitado"' + lastPart + ',"justificacao":"' + text + '"}';
                localStorage
                ...
                m = n.getMonth() + 1;
                d = n.getDate();
                var dataRejeicao = d + "/" + m + "/" + y;
                var horaRejeicao = new Date().toLocaleTimeString();
                localStorage.setItem('dataRejeicao - ' + itemName, '{"data":"' + dataRejeicao + '","hora":"' + horaRejeicao + '"}');
...
function aceitarPedido(num, text){
            ...
            if (pedidosCounter3==num){
                ...
                for (j=0; j<localStorage.length; j++){
                    ...
                    if (func.includes('"tipo":"login"')){
                        ...
                        funcID = func.substring(indexArray3[5]+2,indexArray4[5]-1);
                    }
                }
                ...
                var firstPart = item.substring(0, indexArray[15]+1);
                var lastPart = item.substring(indexArray2[25], item.length-1);
                var newItem = firstPart + '"confirmadoSemContrato"' + lastPart + ',"confirmacao":"' + text + '",' + '"organizador":"' + funcID + '"}';
                localStorage.removeItem(item);
                localStorage.setItem(itemName, newItem);
                console.log(newItem);
                ...
                m = n.getMonth() + 1;
                d = n.getDate();
                var dataSemContrato = d + "/" + m + "/" + y;
                var horaSemContrato = new Date().toLocaleTimeString();
                localStorage.setItem('dataConfirmacaoSemContrato - ' + itemName, '{"data":"' + dataSemContrato + '","hora":"' + horaSemContrato + '"}');
            }
        }
    }
    location.reload();
