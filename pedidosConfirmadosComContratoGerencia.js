...
function displayPedidos(){
    ...
    for (i=0; i<localStorage.length; i++){
        ...
        if (item.includes('"tipo":"pedido","estado":"confirmadoComContrato"') && item.includes('"organizador":"' + funcID + '"')){
            nenhumPedidoFlag = 1;
            var clientID = itemName.substring(23,29);
            listaIDsClientes.push(clientID);
            var clientNome;
            var clientSobrenome;
            var clientEmail;
            var clientTelemovel;
                    ...
                    if (item2.substring(indexArray3[5]+2,indexArray4[5]-1)==clientID){
                        ...
                        for (p=0; p<localStorage.length; p++){
                            ...
                            if (itemName3.includes('dataConfirmacaoSemContrato - '+itemName)){
                                ...
                                b2.innerText = "Confirmação sem contrato: ";
                                p3.appendChild(b2);
                        ...
                        for (p=0; p<localStorage.length; p++){
                            var item3 = localStorage.getItem(localStorage.key(p));
                            var itemName3 = localStorage.key(p);
                            if (itemName3.includes('dataConfirmacaoComContrato - '+itemName)){
                                var rowDiv2 = document.createElement("div");
                                rowDiv2.className = "row";
                                ...
                                p3.style = "padding-left: 2%; padding-right: 1%; width: 25%";
                                var b2 = document.createElement("b");
                                b2.innerText = "Confirmação com contrato: ";
                                p3.appendChild(b2);
                                rowDiv2.appendChild(p3);

                                var p4 = document.createElement("p");   
                                p4.id = "dataComContrato" + pedidosCounter;
                                p4.className = "pedidosElement";
                                p4.innerText = item3.substring(9,18) + " " + item3.substring(28,36);
                                rowDiv2.appendChild(p4);
                        ...
                        btn.setAttribute("id", "darPorTerminado" + pedidosCounter);
                        btn.setAttribute("type", "button");
                        btn.setAttribute("value", "Dar Por Terminado");
                        btn.setAttribute("class", "btn btn-light");
                        btn.setAttribute("style", "margin-right:1%");
                        newDiv.appendChild(btn);

                        var btn2 = document.createElement("input");
                        btn2.setAttribute("id", "contactarCliente" + pedidosCounter);
                        btn2.setAttribute("type", "button");
                        btn2.setAttribute("value", "Contactar Cliente");
                        btn2.setAttribute("class", "btn btn-light");
                        btn2.setAttribute("name", clientID);
                        newDiv.appendChild(btn2);

                        var currentDiv = document.getElementById("todosOsPedidos"); 
                        document.body.insertBefore(newDiv, currentDiv);

                        document.getElementById("darPorTerminado" + pedidosCounter).onclick = function() {darPorTerminado(parseInt(this.id.substring(15)))};
                        document.getElementById("contactarCliente" + pedidosCounter).onclick = function() {
                            // ...
                            var flag = 0;
...
function darPorTerminado(num){
    var pedidosCounter3 = 0;
    for (i=0; i<localStorage.length; i++){
        var item = localStorage.getItem(localStorage.key(i));
        var itemName = localStorage.key(i);
        if (item.includes('"tipo":"pedido","estado":"confirmadoComContrato"')){
            pedidosCounter3++;
            ...
            if (pedidosCounter3==num){
                ...
                var newItem = firstPart + '"terminado"' + lastPart;
                ...
                m = n.getMonth() + 1;
                d = n.getDate();
                var dataDarPorTerminado = d + "/" + m + "/" + y;
                var horaDarPorTerminado = new Date().toLocaleTimeString();
                localStorage.setItem('dataDarPorTerminado - ' + itemName, '{"data":"' + dataDarPorTerminado + '","hora":"' + horaDarPorTerminado + '"}');
            }
        }
    }
    location.reload();
